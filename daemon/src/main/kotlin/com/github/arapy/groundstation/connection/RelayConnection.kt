...
class RelayConnection(
            ...
            try {
                client.webSocket(url) {
                    ...
                    coroutineScope {
                        ...
                        controller.signals
                            .onEach { signal -> sendPayload(WebsocketPayload.DecodedSignalReceived(identity.id, signal)) }
                            .launchIn(this)
    ...
    private suspend fun handleIncoming(payload: WebsocketPayload) {
        when (payload) {
            ...
            is WebsocketPayload.ListenFrequency -> {
                if (payload.stationId == identity.id) controller.listenFrequency(payload.frequencyHz, payload.mode)
            }

            is WebsocketPayload.StopListening -> {
                if (payload.stationId == identity.id) controller.stopListening()
            }
